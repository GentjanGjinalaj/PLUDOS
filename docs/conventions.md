...
Defined tags: `[NETWORK]`, `[SENSOR]`, `[BUFFER]`, `[FSM]`, `[BEACON]`, `[SYSTEM]`.

(`[COAP]` is retired — the CoAP stack was removed by ADR-015.)
...
- Named volumes preferred over host bind-mounts. Exceptions: `/dev/net/tun` for Tailscale; `./ram_buffer` for the data-engine Parquet buffer (host bind-mount intentional — survives container restart).
