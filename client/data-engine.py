...
# ==========================================
# The STM32 is responsible for reporting mission lifecycle events.
# Jetson flushes when the mission is complete, or when its own buffer fill level is high.
JETSON_HARD_LIMIT = 500
JETSON_SOFT_LIMIT_PCT = 0.8  # flush when 80% of Jetson's local buffer capacity is reached
...
class TelemetryResource(resource.Resource):
    ...
    async def render_post(self, request):
        ...
        try:
            # Log request path for debugging (vib/telemetry etc)
            ...
            logger.info(f"Parsed shuttle_id: {shuttle_id}, packet_num: {packet_num}")
            ...
            # --- D. MISSION-DRIVEN FLUSH LOGIC ---
            try:
                if not mission_active:
                    grand_total = mission_energy_tracker.get(shuttle_id, 0.0)
                    logger.info(f"🏁 MISSION COMPLETE (STM32 signaled). Total Energy: {grand_total:.2f} mJ. Flushing buffer.")
                    self.flush_to_storage()
                    mission_energy_tracker[shuttle_id] = 0.0

                elif jetson_buffer_size >= int(JETSON_HARD_LIMIT * JETSON_SOFT_LIMIT_PCT):
                    logger.warning(f"WARN: Jetson RAM buffer reached soft limit ({jetson_buffer_size} pkts). Flushing to protect performance.")
                    self.flush_to_storage()
                    mission_energy_tracker[shuttle_id] = 0.0
                ...
                logger.error(f"Flush failed during mission control: {e}")
