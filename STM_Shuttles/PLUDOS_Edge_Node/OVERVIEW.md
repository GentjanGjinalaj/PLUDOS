...
|------|----------------|--------|
| `Core/Src/main.c` | **The whole application.** ~1900 lines. Holds the IDLE/MOVING state machine, the ISM330DHCX IMU read path (accel + gyro), the 24-byte `PludosTelemetry_t` packer, the boot-time **beacon discovery** that finds the gateway's IP, the live UDP transmit loop, and sequence/timestamp bookkeeping. Also the **high-rate capture/drain path** (ADR-020/021): batches MOVING IMU into the ISM330 FIFO → PSRAM, then drains sealed missions and 10-min IDLE snapshots over UDP, stamping each `DrainBegin` with `t0_tick` and `tx_tick` so the gateway can recover capture time. A boot **reset-cause report** (`[BOOT] reset cause: …`) logs why the MCU last reset, and a **pre-TX jitter** (random 1–15 s before powering the radio for a drain) decorrelates shuttles that exit MOVING together so they don't collide on the shared 2.4 GHz channel. During IDLE the MCU enters **Stop2 deep sleep** (Phase 3), woken by the **ISM330 wake-on-motion hardware INT** or an **RTC alarm** for the 10-min snapshot — the wake threshold sits *under* `MOVEMENT_THRESHOLD_G2` so the FSM still makes the authoritative IDLE→MOVING call after a hardware wake (see `docs/energy_lpm_design.md`). Resolves its own `shuttle_id` at boot from the factory MCU UID (`Shuttle_ResolveId`), and during an IDLE radio window triggers an **OTA pull** (`Ota_TryUpdate`) when the gateway beacon advertises a newer `fw=`. PLUDOS code lives inside the `USER CODE BEGIN/END` guards. | **Core / critical** — this is the firmware |
| `Core/Src/ota.c`, `Core/Inc/ota.h` | **OTA firmware-update receiver** (ADR-019, test/bench tier) — the reverse of the drain. Requests an image from the gateway (UDP 5685, NAK selective-repeat ARQ), stages chunks in PSRAM, runs a whole-image CRC32 gate, then self-flashes the **inactive** flash bank, read-back-verifies, flips `SWAP_BANK` and resets into the new image. A confirm-or-revert state record (`Ota_BootCheck`/`Ota_Confirm`) auto-reverts the swap if a trial image fails to self-confirm. One `firmware.bin` serves the whole fleet — identity comes from the UID, not the image. **Note:** the inactive-bank addressing / `SWAP_BANK` semantics still need an RM0456 cross-check (see header). | Core (update path) |
| `Core/Src/psram.c`, `Core/Inc/psram.h` | External **OCTOSPI PSRAM** driver + memory-map carve-out (capture ring, 16 KB crash-recovery persist region, OTA staging region). Memory-mapped so capture/drain and OTA stage to it directly. | Core (capture + OTA backing store) |
